    ...
    return (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2 mt-3 [&>*]:min-w-0">
            ...
            {Number(selectedProduct.PUPorMayor) > 0 && (
                <button type="button" onClick={() => onPriceTypeChange('porMayor')}
                        className={`relative rounded-xl p-2 sm:p-3 text-center transition-all border-2 ${priceType === 'porMayor' ? 'border-violet-500 bg-violet-50 dark:bg-violet-900/30 shadow-sm' : 'border-violet-200 dark:border-violet-800 bg-violet-50/50'}`}>
            ...
            {Number(selectedProduct.PUPorMenor) > 0 && (
                <button type="button" onClick={() => onPriceTypeChange('porMenor')}
                        className={`relative rounded-xl p-2 sm:p-3 text-center transition-all border-2 ${priceType === 'porMenor' ? 'border-green-500 bg-green-50 dark:bg-green-900/30 shadow-sm' : 'border-green-200 dark:border-green-800 bg-green-50/50'}`}>
            ...
            <button type="button" onClick={() => onPriceTypeChange('custom')}
                    className={`relative rounded-xl p-2 sm:p-3 text-center transition-all border-2 ${priceType === 'custom' ? 'border-red-500 bg-red-50 dark:bg-red-900/30 shadow-sm' : 'border-red-200 dark:border-red-800 bg-red-50/50'}`}>
                ...
                <div className="flex items-center justify-center gap-0.5">
                    ...
                    <Input
                        ...
                        onBlur={(e) => {
                            ...
                            if (!val || val <= 0) {
                                onPriceEditChange(Number(selectedProduct.PUContado))
                            } else {
                                onPriceEditBlur(e)
                            }
                        }}
                ...
                {priceType === 'custom' && (Number(priceEdit) <= 0) && (
                    <p className="text-[9px] text-red-500 mt-0.5">Debe ser mayor a 0</p>
                )}
            </button>

            <button type="button" onClick={() => onPriceTypeChange('regalo')}
                    className={`relative rounded-xl p-2 sm:p-3 text-center transition-all border-2 ${priceType === 'regalo' ? 'border-pink-500 bg-pink-50 dark:bg-pink-900/30 shadow-sm' : 'border-pink-200 dark:border-pink-800 bg-pink-50/50'}`}>
                {priceType === 'regalo' && <Check className="absolute top-1 right-1 h-3 w-3 text-pink-600 dark:text-pink-400" />}
                <div className="flex flex-col items-center gap-0.5">
                    <Gift className="h-3.5 w-3.5 text-pink-500 dark:text-pink-400" />
                    <div className="text-[10px] sm:text-xs font-medium text-pink-600 dark:text-pink-400">Regalo</div>
                </div>
            </button>
        </div>
    )
