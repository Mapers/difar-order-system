  ...
  <div className="rounded-md border bg-background">
    <div className="grid grid-cols-2 sm:grid-cols-4 w-full mb-2 gap-4 px-4 sm:px-6">
      {[...Array(4)].map((_, i) => (
        <div
          key={i}
          className="h-8 bg-muted rounded-md animate-pulse my-2"
          style={{ minWidth: "80px" }}
        ></div>
      ))}
    </div>
    ...
    <div className="bg-gradient-to-r from-indigo-600 to-indigo-800 px-4 sm:px-6 py-4 shadow-md mb-4">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
        <div className="h-6 sm:h-7 w-24 bg-muted rounded animate-pulse"></div>
        <div className="flex flex-col sm:items-end gap-1">
          <div className="h-5 sm:h-6 w-40 bg-muted rounded animate-pulse"></div>
        </div>
      </div>
    </div>
    ...
    <div className="mt-4 space-y-4 px-4 sm:px-6 py-1">
      {[...Array(1)].map((_, i) => (
        <div
          key={i}
          className="flex flex-col md:flex-row md:justify-between md:items-center gap-2 p-4 bg-muted rounded-md animate-pulse"
        >
      ...
      <Card className="bg-blue-100 border-blue-300">
        ...
        <CardContent className="space-y-3 text-sm">
          {Array.from({ length: 5 }).map((_, idx) => (
            <div key={idx}>
              <div className="h-3 w-24 mb-1 bg-muted rounded animate-pulse" />
      ...
      <Card className="bg-green-100 border-green-300">
        ...
        <CardContent className="space-y-3 text-sm">
          {Array.from({ length: 4 }).map((_, idx) => (
            <div key={idx}>
              <div className="h-3 w-28 mb-1 bg-muted rounded animate-pulse" />
              <div className="h-4 w-full bg-muted rounded animate-pulse" />
            </div>
          ))}
        </CardContent>
