...
export const ExportExpiredBalancesPdf: React.FC<ExportPdfProps> = ({ data, disabled = false }) => {
    ...
    const generatePdf = async () => {
        ...
        try {
            ...
            const cols = [
                { name: "Emisión", w: 50, align: 'left' },
                { name: "Vcto.", w: 50, align: 'left' },
                { name: "Doc", w: 35, align: 'center' },
                { name: "Nro Doc", w: 75, align: 'left' },
                { name: "Mon", w: 25, align: 'center' },
                { name: "Provisión", w: 65, align: 'right' },
                { name: "Amortiza", w: 65, align: 'right' },
                { name: "Saldo S/.", w: 75, align: 'right' },
                { name: "Saldo US$", w: 75, align: 'right' }
            ];
            ...
            for (const vendedor of data) {
                ...
                for (const zona of vendedor.zonas) {
                    ...
                    for (const cliente of zona.clientes) {
                        ...
                        let startX = margin + cols[0].w + cols[1].w + cols[2].w + cols[3].w + cols[4].w + cols[5].w + cols[6].w;

                        currentPage.drawText(txtTotalCl, { x: startX - boldFont.widthOfTextAtSize(txtTotalCl, 8) - 10, y: yPosition, size: 8, font: boldFont });
                        currentPage.drawText(txtSoles, { x: startX + cols[7].w - boldFont.widthOfTextAtSize(txtSoles, 8) - 3, y: yPosition, size: 8, font: boldFont, color: colors.danger });
                        currentPage.drawText(txtDolares, { x: startX + cols[7].w + cols[8].w - boldFont.widthOfTextAtSize(txtDolares, 8) - 3, y: yPosition, size: 8, font: boldFont, color: colors.danger });
                    ...
                    let stXZona = margin + cols[0].w + cols[1].w + cols[2].w + cols[3].w + cols[4].w + cols[5].w + cols[6].w;
                    ...
                    currentPage.drawText(txtZonaS, { x: stXZona + cols[7].w - boldFont.widthOfTextAtSize(txtZonaS, 9) - 3, y: yPosition, size: 9, font: boldFont, color: colors.primary });
