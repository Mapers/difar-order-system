...
function EstadoIcon({ estado, hora, monto }: { estado: EstadoVisita; hora: string | null; monto: number }) {
    ...
    if (estado === 'VISITADO_SIN_PEDIDO') {
        return <CheckCircle className="h-3.5 w-3.5 text-slate-400 flex-shrink-0" />;
    }
        ...
        <Dialog open={open} onOpenChange={onClose}>
            <DialogContent className="max-w-lg max-h-[90vh] overflow-hidden flex flex-col p-0">
                <div className="p-4 pb-3 border-b border-slate-200">
                    ...
                    <p className="text-[11px] text-slate-500 mt-0.5">Semana {semanaLabel}</p>
                ...
                <div className="flex-1 overflow-y-auto p-3 space-y-3">
                    {data.detalle.length === 0 ? (
                        <p className="text-center text-slate-400 text-sm py-8">Sin datos de visitas</p>
                    ) : (
                                ...
                                <div className="space-y-1 pl-2">
                                    {dia.clientes.map((cl, idx) => (
                                        <div key={`${cl.cod_cliente}-${idx}`}
                                             className="flex items-start gap-2 px-2 py-1.5 rounded hover:bg-slate-50 transition-colors">
                                            <div className="mt-0.5">
                                                <EstadoIcon estado={cl.estado_visita} hora={cl.hora_visita} monto={cl.monto_pedido} />
                                            </div>
                                            <div className="flex-1 min-w-0">
                                                <p className={`text-xs font-medium truncate ${
                                                    cl.estado_visita === 'PROGRAMADO' ? 'text-slate-400'
                                                ...
                                                <div className="flex items-center gap-2 mt-0.5">
                                                    {cl.estado_visita === 'PROGRAMADO' && (
                                                        <span className="text-[9px] text-slate-400">Programado</span>
                                                    )}
                                                    ...
                                                    {cl.estado_visita === 'VISITADO_SIN_PEDIDO' && (
                                                        <span className="text-[9px] text-slate-400">sin pedido</span>
                                                    )}
                                                </div>
                                            </div>
                                        </div>
