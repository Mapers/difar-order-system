                ...
                <div className="flex-1 overflow-y-auto p-4">
                    {loading ? (
                        ...
                        <div className="space-y-2">
                            <div className="grid grid-cols-[1fr_90px_90px] gap-2 px-3 py-1.5 text-[9px] uppercase tracking-wider text-slate-400 font-semibold">
                                <span>Cliente</span>
                            ...
                            {clientes.map((c) => (
                                <div key={c.cod_cliente}
                                     className="grid grid-cols-[1fr_90px_90px] gap-2 px-3 py-2.5 rounded-lg bg-slate-50 hover:bg-slate-100 items-center transition-colors">
                                    <div className="min-w-0">
                                        <p className="text-xs font-medium text-slate-800 truncate" title={c.nombre_cliente}>
                                            {c.nombre_cliente || c.cod_cliente}
                                        </p>
                                        <p className="text-[10px] text-slate-400">{c.cod_cliente}</p>
                                    </div>
                            ...
                            <div className="grid grid-cols-[1fr_90px_90px] gap-2 px-3 py-2.5 bg-slate-100 rounded-lg border border-slate-200 items-center mt-2">
                                <span className="text-[11px] font-semibold text-slate-500">TOTAL · {clientes.length} clientes</span>
                                <span></span>
                                <span className="text-right text-xs font-bold text-slate-700">{fmtMoney(totalMonto)}</span>
                            </div>
                        </div>
                    )}
                </div>
