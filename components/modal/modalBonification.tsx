...
interface ModalBonificationProps {
  ...
  currentBonification: ICurrentBonification | null,
  products: IProduct[],
...
const ModalBonification: React.FC<ModalBonificationProps> = ({
  ...
  const handleConfirmBonification = () => {
    onOpenChange(false)
    addProductToList()
    if (currentBonification && selectedBonifications.length > 0) {
      selectedBonifications.forEach((bonificationId) => {
        const bonificacion = currentBonification.bonificaciones.find((b: any) => b.ProductoBonificado === bonificationId)
        if (bonificacion) {
          const bonificadoProducto = products.find((p: any) => p.Codigo_Art === bonificacion.ProductoBonificado)
          if (bonificadoProducto) {
            setTimeout(() => {
              setSelectedProducts((prev: any) => [
                ...prev,
                {
                  product: bonificadoProducto,
                  quantity: bonificacion.TotalBonificados,
                  isBonification: true,
                  bonificationId: bonificacion.ProductoBonificado,
                  finalPrice: 0,
                },
              ])
            }, 800)
                  ...
                  <div className="space-y-2">
                    {selectedBonifications.map((bonificationId) => {
                      const bonificacion = currentBonification.bonificaciones.find(
                        (b: any) => b.ProductoBonificado === bonificationId
                      );
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            +{parseInt(bonificacion.TotalBonificados)} unidades
                          </Badge>
                        </div>
                      );
                    })}
