    ...
    return (
        <div className="grid gap-6 p-4 md:p-6">
            <div className="flex flex-col gap-2">
                <h1 className="text-2xl md:text-3xl font-bold tracking-tight text-gray-900 dark:text-gray-100">Saldo de Documentos por Cobrar</h1>
                <p className="text-sm md:text-base text-gray-500 dark:text-gray-400">Consulta los saldos pendientes agrupados por vendedor y zona.</p>
            </div>
            ...
            <Card className="shadow-md">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                        ...
                        <div className="flex flex-col gap-1 md:col-span-3">
                            ...
                            <Popover open={openZone} onOpenChange={setOpenZone}>
                                <PopoverTrigger asChild>
                                    <Button variant="outline" role="combobox" className="justify-between w-full h-10 font-normal bg-white dark:bg-gray-900">
                                        <span className="truncate">
                ...
                <CardContent className="p-4 md:p-6 bg-slate-50 dark:bg-gray-800">
                    {loading ? (
                        <div className="flex flex-col items-center justify-center py-16">
                            <div className="animate-spin rounded-full h-10 w-10 border-b-2 border-blue-600 mb-4"></div>
                            <p className="text-slate-500 dark:text-gray-400 font-medium">Buscando documentos, por favor espera...</p>
                        </div>
                    ) : data ? (
                        <div className="space-y-8">
                            ...
                            {data.Clientes.map((cliente: any, cIdx: number) => (
                                <div key={cIdx} className="space-y-4">
                                    <div className="bg-white dark:bg-gray-900 p-4 rounded-lg border border-slate-200 dark:border-gray-700 shadow-sm grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            <h2 className="text-lg font-bold text-slate-800 dark:text-gray-200 flex items-center gap-2">{cliente.Cliente}</h2>
                                            <p className="text-sm text-slate-600 dark:text-gray-400 mt-1"><span className="font-semibold">RUC:</span> {cliente.RUC}</p>
                                            <p className="text-sm text-slate-600 dark:text-gray-400 flex items-center gap-1 mt-1">{cliente.NombreComercial}</p>
                                        </div>
                                        <div className="md:text-right space-y-1">
                                            <p className="text-sm text-slate-600 dark:text-gray-400 flex items-center md:justify-end gap-1">{cliente.Direccion}</p>
                                            <p className="text-sm text-slate-600 dark:text-gray-400 flex items-center md:justify-end gap-1">{cliente.Telefono || 'N/A'}</p>
                                        </div>
                                    </div>

                                    {cliente.vendedores.map((vend: any, vIdx: number) => (
                                        <div key={vIdx} className="border border-slate-300 dark:border-gray-700 rounded-lg overflow-hidden shadow-sm bg-white dark:bg-gray-900 ml-0 md:ml-4">
                                            ...
                                            <div className="hidden md:block overflow-x-auto">
                                                <table className="w-full text-xs text-left text-slate-600 dark:text-gray-400">
                                                    <thead className="text-[10px] md:text-xs text-slate-500 dark:text-gray-400 uppercase bg-slate-100 dark:bg-gray-800 border-b border-slate-200 dark:border-gray-700">
                                                    <tr>
                                                    ...
                                                    {vend.documentos.map((doc: any, dIdx: number) => {
                                                        ...
                                                        return (
                                                            <tr key={dIdx} className="hover:bg-slate-50 dark:hover:bg-gray-700/30 transition-colors">
                                                                ...
                                                                <td className="px-3 py-2 text-right font-bold text-blue-700 bg-blue-50/30 dark:bg-blue-900/20">
                                                                    {isSoles ? formatMoney(doc.Saldo) : '-'}
                                                                </td>
                                                                <td className="px-3 py-2 text-right font-bold text-green-700 bg-green-50/30 dark:bg-green-900/20">
                                                                    {!isSoles ? formatMoney(doc.Saldo) : '-'}
                                                                </td>
                                                            </tr>
                                                        )
                                            ...
                                            <div className="grid grid-cols-1 gap-3 p-3 md:hidden bg-slate-50 dark:bg-gray-800">
                                                {vend.documentos.map((doc: any, dIdx: number) => {
                                                    const isSoles = doc.Tipo_Moneda === 'NSO' || doc.Moneda === 'S/.';
                                                    return (
                                                        <div key={dIdx} className="bg-white dark:bg-gray-900 border border-slate-200 dark:border-gray-700 rounded-lg p-3 shadow-sm flex flex-col gap-3">
                                                            <div className="flex justify-between items-start border-b border-slate-100 dark:border-gray-700 pb-2">
                                                                <div>
                                                                    <span className="text-[10px] uppercase font-bold text-slate-400 dark:text-gray-500">{doc.TipoDocumento}</span>
                                                                    <p className="font-mono font-bold text-sm text-slate-800 dark:text-gray-200">{doc.SerieDoc}-{doc.NumeroDoc}</p>
                                                                </div>
                                                            ...
                                                            <div className="grid grid-cols-2 gap-2 text-xs">
                                                                <div>
                                                                    <span className="text-slate-500 dark:text-gray-400">Emisión:</span>
                                                                    <p className="font-medium text-slate-700 dark:text-gray-300">{formatDateToDDMMYYYY(doc.Fecha_Emision)}</p>
                                                                </div>
                                                                <div>
                                                                    <span className="text-slate-500 dark:text-gray-400">Vcto:</span>
                                                                    <p className="font-medium text-red-600">{formatDateToDDMMYYYY(doc.Fecha_Vcto)}</p>
                                                                </div>
                                                                <div>
                                                                    <span className="text-slate-500 dark:text-gray-400">Prov:</span>
                                                                    <p className="font-medium">{formatMoney(doc.SumaProvision)}</p>
                                                                </div>
                                                                <div>
                                                                    <span className="text-slate-500 dark:text-gray-400">Amort:</span>
                                                            ...
                                                            <div className="flex justify-between items-center pt-2 border-t border-slate-100 dark:border-gray-700">
                                                                <span className="text-xs font-bold uppercase text-slate-500 dark:text-gray-400">Saldo Final:</span>
                            ...
                            <div className="bg-white dark:bg-gray-900 border border-blue-200 dark:border-blue-700 p-4 rounded-lg flex flex-col sm:flex-row justify-between items-center shadow-md mt-6 sticky bottom-4">
                                <span className="text-sm font-bold uppercase tracking-wider text-slate-700 dark:text-gray-300 mb-2 sm:mb-0">Deuda Total Global</span>
                                <div className="flex gap-4 sm:gap-6 w-full sm:w-auto justify-between sm:justify-end">
                                    <div className="text-center sm:text-right bg-blue-50 dark:bg-blue-900/20 p-2 rounded-md sm:bg-transparent sm:p-0 flex-1 sm:flex-none border border-blue-100 dark:border-blue-700 sm:border-none">
                                    ...
                                    <div className="text-center sm:text-right bg-green-50 dark:bg-green-900/20 p-2 rounded-md sm:bg-transparent sm:p-0 flex-1 sm:flex-none border border-green-100 dark:border-green-700 sm:border-none">
