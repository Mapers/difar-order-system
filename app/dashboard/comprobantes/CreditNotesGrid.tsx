...
import {
    ...
    MessageCircle,
    Activity,
    FileDiff, Info, Ban, CalendarDays, UserRound
} from "lucide-react"
...
interface CreditNotesGridProps {
    notas: Comprobante[]
    loading: boolean
...
// Vista alternativa a CreditNotesTable: mismos datos y acciones, en tarjetas
// (3 por fila en desktop) en vez de tabla.
export function CreditNotesGrid({
                                     notas,
                                     loading,
                                     tiposComprobante,
                                     isAdmin,
    ...
    if (notas.length === 0) {
        return <div className="text-center py-12 text-muted-foreground">No se encontraron notas de crédito</div>
    }
                    ...
                    return (
                        <Card
                            ...
                            className={cn(
                                ...
                                noUtilizado && "opacity-60"
                            )}
                        >
                            <CardContent className="flex flex-1 flex-col p-0">
                                <div className="flex items-start justify-between gap-2 border-b border-border bg-muted/40 p-4">
                                    <div className="min-w-0 flex items-start gap-2.5">
                                        <div className="mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-purple-100 text-purple-600">
                                ...
                                <div className="flex flex-1 flex-col gap-3 p-4">
                                    <div>
                                        <p
                                            className={cn(
                                                ...
                                                noUtilizado ? "italic text-amber-700" : "text-foreground"
                                            )}
                                            title={nota.cliente_denominacion ?? ''}
                                        >
                                    ...
                                    <div className="flex flex-col gap-1 text-xs text-muted-foreground">
                                        <div className="flex items-center gap-1.5">
                                            ...
                                            <span className="truncate">{formatFecha(nota.fecha_envio)}</span>
                                        </div>
                                        {nota.Vendedor && (
                                            <div className="flex items-center gap-1.5">
                                                <UserRound className="h-3.5 w-3.5 shrink-0" />
                                                <span className="truncate">{nota.Vendedor}</span>
                                            </div>
                                        )}
                                    </div>
                                    ...
                                    <div className="mt-auto flex items-end justify-between gap-2 border-t border-border pt-3">
                                        <div>
                                            <p className="text-[10px] uppercase tracking-wide text-muted-foreground">Total Devuelto</p>
                                            {noUtilizado ? (
